...
func (c *Conn) clientHandshake() (err error) {
	...
	if c.config.GMSupport != nil {
		c.vers = VersionGMSSL
		hello, params, err = c.makeClientHelloGM()
	} else {
		hello, params, err = c.makeClientHello()
	}
	if err != nil {
		return err
	}
	...
	if c.vers == VersionTLS13 {
		...
		return hs.handshake()
	} else if c.config.GMSupport != nil {
...
func (c *Conn) pickTLSVersion(serverHello *serverHelloMsg) error {
	...
	var ok 	 bool
	if c.config.GMSupport != nil {
		vers, ok = c.config.GMSupport.mutualVersionGM([]uint16{peerVersion})
	} else {
...
func (c *Conn) loadSession(hello *clientHelloMsg) (cacheKey string,
	...
	if session.vers != VersionTLS13 {
		...
		if c.config.GMSupport != nil {
			if mutualCipherSuiteGM(hello.cipherSuites, session.cipherSuite) == nil {
				return cacheKey, nil, nil, nil
			}
		}else if mutualCipherSuite(hello.cipherSuites, session.cipherSuite) == nil {
			return cacheKey, nil, nil, nil
		}
